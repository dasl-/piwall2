...
Or:
....
pi@piwall6.local: Fetched 91.3 MB in 53s (1,723 kB/s)
pi@piwall6.local: E: Failed to fetch http://raspbian.mirror.constant.com/raspbian/pool/main/l/luajit/luajit_2.1.0~beta3+dfsg-5.3+deb11u1_armhf.deb  Error reading from server - read (104: Connection reset by peer) [IP: 108.61.5.87 80]
pi@piwall6.local: E: Unable to fetch some archives, maybe run apt-get update or try with --fix-missing?
pi@piwall6.local: Error in install_dependencies.sh at line number: 100 with exit code: 100
....
...
**idea for next time**: disable ethernet power save before rebooting pis and see if that fixes it: https://github.com/raspberrypi/linux/issues/4289
