...
* wifi internet becomes sporadically slow. I have periods of normal speeds that last maybe ~5 minutes, followed by what seems like an internet drop out that lasts for ~1 minute. I am not 100% sure that this symptom was related to the caused by the issue - it could have been a coincidence.
* I am able to access my router via http://192.168.1.1 , and I am able to ssh onto some pis on my network, such as pifi.local, however ssh'ing onto any of the piwall raspberry pis is very slow
* Running this dsh command shows that only one piwall raspberry pi is totally unresponsive: piwall3.local. The rest of them do respond, however it takes a minute or two for them to respond:
....
...
Based on git blame, the original occurence of this issue øccurred on 2022-01-06, which was 1.5 years ago. Given that the piwall has been running 24/7, a bug that occurs once every 1.5 years seems not the worst thing in the world. I suspect some sort of linux kernel related bug.
