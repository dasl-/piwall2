...
Note on OS support (https://pip.raspberrypi.com/categories/685-whitepapers-app-notes/documents/RP-004341-WP/Troubleshooting-KMS-HDMI-output.pdf[reference]):
...
* Bullseye: In bullseye, the default graphics stack is KMS. In order to use the legacy graphics stack, we must comment out `dtoverlay=vc4-kms-v3d`
* Bookworm: In bookworm, only the KMS graphics stack is supported. Neither FKMS nor legacy graphics stacks work. Thus, bookworm is incompatible (AFAIK) with performant screen rotation.
on buster, we need to comment out `dtoverlay=vc4-fkms-v3d`, but on bullseye, we need to comment out `dtoverlay=vc4-kms-v3d` (`fkms` vs `kms`).
