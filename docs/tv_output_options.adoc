...
To get around this playback speed sync issue, I tried adjusting playback speed via `omxplayer`'s https://github.com/popcornmix/omxplayer/#rate-rw[DBUS API], but this proved to be a losing battle. As the amount of lag seemed to vary with video resolution, I'd need to calculate lag compensation amounts for an endless number of video resolutions. I tried investigating `omxplayer`'s https://github.com/dasl-/piwall2/blob/main/docs/configuring_omxplayer.adoc#live[`--live` option again]. This actually sort of worked, but the jankiness we've seen in the past when using this option continued. Sound was full of static, and occasionally there would be video glitch artifacts (green screens, slight sync issues, etc).
